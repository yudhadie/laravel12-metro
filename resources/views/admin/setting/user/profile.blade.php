...
@section('content')
    <x-admin.card.default>
        <x-admin.form.edit action="{{ route('user.update', $data->id) }}" enctype="multipart/form-data">
            ...
            <x-admin.form.input-right label="Photo Profile" name="photo" type="file" value="{{ $data->photo }}"
                accept=".jpeg,.jpg,.png">
                @if ($data->photo != null)
                    <img class="mw-100 mh-300px card-rounded mb-2" src="{{ asset($data->photo) }}" />
                    @isset($data->photo)
                        <div class="mt-1">
                            <button class="btn btn-danger btn-sm mb-2" href="{{ route('delete-photo-user', $data->id) }}"
                                id="delete">
                                Delete
                            </button>
                        </div>
